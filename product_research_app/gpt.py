...
# -----------------------------------------------------------------------------
# Analyse a single product using the OpenAI chat API
#
# Given a product dictionary with keys ``name``, ``description``, ``category`` and
# optional ``extras`` (a mapping of additional attributes), this function
# constructs a prompt based on the research principles contained in the user
# provided documents (Breakthrough Advertising, MasterClass, etc.).  The
# assistant is instructed to produce a JSON object with specific fields that
# summarise the strengths and weaknesses of the product, typical consumer
# opinions, notable competitors and any other relevant insights.  The
# returned object contains ``pros``, ``cons``, ``opinions``, ``competitors`` and
...
def analyze_product(api_key: str, model: str, product: Dict[str, Any], *, temperature: float = 0.2) -> Dict[str, Any]:
    ...
    Args:
        ...
        model: Identifier of the model to call (e.g. ``gpt-4o``).
        product: A dictionary representing the product.  Should include at
            minimum a ``name`` field; optional fields ``description``,
            ``category`` and ``extras`` provide additional context.
        temperature: Sampling temperature for the generation.  Lower values
    ...
    extra_parts = []
    # include some of the more informative extras
    for key in ["Price($)", "Item Sold", "Revenue($)", "Product Rating", "Launch Date", "Creator Conversion Ratio"]:
        val = extras.get(key)
        if val:
            extra_parts.append(f"{key}: {val}")
    extra_str = "; ".join(extra_parts)
    info = f"Nombre: {product['name']}. "
    if descr:
        info += f"Descripción: {descr}. "
    if cat:
    ...
    messages = [
        ...
        {"role": "user", "content": f"Producto a analizar:\n{info}\n{user_prompt}"},
    ]
    try:
        resp = call_openai_chat(api_key, model, messages, temperature=temperature)

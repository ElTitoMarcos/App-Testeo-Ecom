    ...
    try:
        ...
        if records:
            ...
            for row in records:
                ...
                fecha_lanz = None
                if launch_col:
                    raw_launch = row.get(launch_col)
                    if isinstance(raw_launch, (datetime, date)):
                        fecha_lanz = raw_launch.strftime('%Y-%m-%d')
                    elif raw_launch not in (None, ''):
                        fecha_lanz = str(raw_launch).strip()
                    if fecha_lanz:
                        extras['fecha_lanzamiento'] = fecha_lanz
                        extras['Launch Date'] = fecha_lanz

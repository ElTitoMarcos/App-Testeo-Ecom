...
{
  echo "=================================="
  echo "$(date) — Bootstrap run_app.sh"
} >>"$LOG_FILE"
...
prepare_embedded_python() {
  ...
  if ! curl -L "$pkg_url" -o "$tmp_pkg"; then
    log_error "No se pudo descargar ${pkg_url}. Verifica tu conexión."
    rm -f "$tmp_pkg"
    return 1
  fi
...
ensure_python() {
  ...
  if [ -x "${ROOT}.venv/bin/python" ]; then
    ...
    PY_VERSION="$(python_version_string "$PYEXE")"
    PY_SOURCE="venv"
    log_info "Reutilizando Python desde .venv (${PY_VERSION})."
    return 0
  fi
